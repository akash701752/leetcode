...
public:
    int numMagicSquaresInside(vector<vector<int>>& grid) {
        ...
        int magicSquareCount = 0;  // counter for magic squares
        // Lambda function to check if the 3x3 grid with top-left corner at (i, j) is a magic square
        auto isMagicSquare = [&](int i, int j) -> int {
            // Check if the square extends beyond the grid boundaries
            if (i + 3 > rows || j + 3 > cols) {
                return 0;
            }
            vector<int> count(16, 0); // Counter for numbers 1 to 9 within the 3x3 grid
            vector<int> sumRow(3, 0); // Sum of each row within the 3x3 grid
            vector<int> sumCol(3, 0); // Sum of each column within the 3x3 grid
            int diagSum1 = 0;         // Sum of the first diagonal
            int diagSum2 = 0;         // Sum of the second diagonal
            // Iterate over the 3x3 grid and populate sums and counts
            for (int x = i; x < i + 3; ++x) {
                for (int y = j; y < j + 3; ++y) {
                    int value = grid[x][y];
                    // Check for invalid numbers or duplicates
                    if (value < 1 || value > 9 || ++count[value] > 1) {
                        return 0;
                    }
        ...
        return magicSquareCount; // Return the total count of magic squares
    }
};
